...
static void print_frame(const UdpFrame *f, uint32_t seq_prev, int verbose) {
    int dropped = 0;
    if (seq_prev != 0xFFFFFFFFu && f->seq != (uint16_t)(seq_prev + 1))
        dropped = (int)(uint16_t)(f->seq - seq_prev) - 1;
    if (dropped > 0)
        printf("  [丢帧 %d]\n", dropped);
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        /* 丢帧检测: seq 未 +1 即视为丢过帧 */
        if (seq_prev != 0xFFFFFFFFu && f->seq != (uint16_t)(seq_prev + 1))
            stat_drop += (uint16_t)(f->seq - seq_prev) - 1;
        seq_prev = f->seq;
        ...
        print_frame(f, seq_prev, verbose);
