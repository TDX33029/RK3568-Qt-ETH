...
static void compute_anchor_geometry(
    ...
    *range = sqrt((*rho) * (*rho) + (*dz) * (*dz));
    if (*rho < TRACKER3D_EPS) *rho = TRACKER3D_EPS;
    if (*range < TRACKER3D_EPS) *range = TRACKER3D_EPS;
}
...
static size_t build_descriptors(
    ...
    for (size_t i = 0; i < config->anchor_count; ++i) {
        if (i == config->ref_anchor) continue;
        if (config->enable_tdoa[i] && measurement->has_tdoa[i]) {
            descriptors[count].kind = OBS_TDOA; descriptors[count].anchor = i; ++count;
        }
    }
    for (size_t i = 0; i < config->anchor_count; ++i) {
        if (config->enable_toa[i] && measurement->has_toa[i]) {
            descriptors[count].kind = OBS_TOA; descriptors[count].anchor = i; ++count;
        }
    }
    for (size_t i = 0; i < config->anchor_count; ++i) {
        if (config->enable_aoa[i] && measurement->has_aoa[i]) {
            descriptors[count].kind = OBS_AOA_AZ; descriptors[count].anchor = i; ++count;
            descriptors[count].kind = OBS_AOA_EL; descriptors[count].anchor = i; ++count;
        }
    }
    for (size_t i = 0; i < config->anchor_count; ++i) {
        if (config->enable_rss[i] && measurement->has_rss[i]) {
            descriptors[count].kind = OBS_RSS; descriptors[count].anchor = i; ++count;
        }
    }
    return count;
}
...
static void fill_single_measurement_model(
    ...
    double dx, dy, dz, rho, range;
    memset(h_row, 0, sizeof(double) * TRACKER3D_STATE_DIM);
    compute_anchor_geometry(state, anchor, &dx, &dy, &dz, &rho, &range);

    switch (descriptor->kind) {
        case OBS_TDOA: {
            const Tracker3DVec3 *ref = &config->anchors[config->ref_anchor];
            double rdx, rdy, rdz, rrho, rrange;
            compute_anchor_geometry(state, ref, &rdx, &rdy, &rdz, &rrho, &rrange);
            *z = measurement->tdoa[anchor_idx] * config->light_speed;
            *h = range - rrange;
            h_row[0] = dx / range - rdx / rrange;
            h_row[1] = dy / range - rdy / rrange;
            h_row[2] = dz / range - rdz / rrange;
        ...
        case OBS_AOA_AZ:
            ...
            *h = atan2(dy, dx);
            h_row[0] = -dy / (rho * rho); h_row[1] = dx / (rho * rho); h_row[2] = 0.0;
        ...
        case OBS_AOA_EL: {
            ...
            *h = atan2(dz, rho);
            h_row[0] = -(dx * dz) / (rho * rsq); h_row[1] = -(dy * dz) / (rho * rsq); h_row[2] = rho / rsq;
            *variance = config->aoa_std_rad * config->aoa_std_rad;
            break;
        }
        case OBS_RSS: {
...
int tracker3d_predict(Tracker3DState *tracker, const Tracker3DConfig *config, double dt) {
    ...
    double pcov[TRACKER3D_STATE_DIM * TRACKER3D_STATE_DIM];
    double q[TRACKER3D_STATE_DIM * TRACKER3D_STATE_DIM];
    double ps[TRACKER3D_STATE_DIM];
    double sa = config->process_accel_std;
    double sa2 = sa * sa;
    double dt2 = dt * dt;
    double dt3 = dt2 * dt;
    double dt4 = dt2 * dt2;
    ...
    for (size_t ax = 0; ax < 3; ++ax) {
        size_t p = ax, v = ax + 3;
        q[p * TRACKER3D_STATE_DIM + p] = 0.25 * dt4 * sa2;
        q[p * TRACKER3D_STATE_DIM + v] = 0.5 * dt3 * sa2;
        q[v * TRACKER3D_STATE_DIM + p] = 0.5 * dt3 * sa2;
        q[v * TRACKER3D_STATE_DIM + v] = dt2 * sa2;
    }

    for (size_t i = 0; i < TRACKER3D_STATE_DIM; ++i) {
        ps[i] = f[i * TRACKER3D_STATE_DIM + 0] * tracker->state[0]
    ...
    mat6_mul(temp, ft, pcov);

    for (size_t i = 0; i < TRACKER3D_STATE_DIM * TRACKER3D_STATE_DIM; ++i)
        tracker->covariance[i] = pcov[i] + q[i];

    memcpy(tracker->state, ps, sizeof(ps));
    return 0;
}

int tracker3d_update(Tracker3DState *tracker, const Tracker3DConfig *config, const Tracker3DMeasurement *measurement) {
    ObservationDescriptor desc[TRACKER3D_MAX_MEAS_DIM];
    size_t dim = build_descriptors(config, measurement, desc);
    if (dim == 0) return 0;

    for (size_t k = 0; k < dim; ++k) {
        double z, h, h_row[TRACKER3D_STATE_DIM], var;
        double pht[TRACKER3D_STATE_DIM], kg[TRACKER3D_STATE_DIM];
        double kh[TRACKER3D_STATE_DIM * TRACKER3D_STATE_DIM];
        double a[TRACKER3D_STATE_DIM * TRACKER3D_STATE_DIM];
        ...
        fill_single_measurement_model(config, measurement, tracker->state, &desc[k], &z, &h, h_row, &var);
        innov = z - h;
        if (desc[k].kind == OBS_AOA_AZ || desc[k].kind == OBS_AOA_EL)
        ...
        s = var;
        for (size_t r = 0; r < TRACKER3D_STATE_DIM; ++r) s += h_row[r] * pht[r];
        if (!std::isfinite(s) || s < 1e-12) return -3;

        for (size_t r = 0; r < TRACKER3D_STATE_DIM; ++r) {
            kg[r] = pht[r] / s;
            tracker->state[r] += kg[r] * innov;
        }

        memset(kh, 0, sizeof(kh));
        for (size_t r = 0; r < TRACKER3D_STATE_DIM; ++r)
            for (size_t c = 0; c < TRACKER3D_STATE_DIM; ++c)
                kh[r * TRACKER3D_STATE_DIM + c] = kg[r] * h_row[c];

        set_identity6(a);
        for (size_t r = 0; r < TRACKER3D_STATE_DIM * TRACKER3D_STATE_DIM; ++r)
            a[r] -= kh[r];

        mat6_mul(a, tracker->covariance, t6);
        {
            double at[TRACKER3D_STATE_DIM * TRACKER3D_STATE_DIM];
            mat6_transpose(a, at);
            mat6_mul(t6, at, t6b);
        }
        ...
        for (size_t r = 0; r < TRACKER3D_STATE_DIM; ++r)
            for (size_t c = 0; c < TRACKER3D_STATE_DIM; ++c)
                krkt[r * TRACKER3D_STATE_DIM + c] = kg[r] * var * kg[c];

        for (size_t r = 0; r < TRACKER3D_STATE_DIM * TRACKER3D_STATE_DIM; ++r)
            tracker->covariance[r] = t6b[r] + krkt[r];

        for (size_t r = 0; r < TRACKER3D_STATE_DIM; ++r)
            for (size_t c = r + 1; c < TRACKER3D_STATE_DIM; ++c) {
...
int tracker3d_step(Tracker3DState *tracker, const Tracker3DConfig *config, const Tracker3DMeasurement *measurement, double dt) {
    if (tracker3d_predict(tracker, config, dt) != 0) return -1;
...
void tracker3d_simulate_measurement(
    ...
    compute_anchor_geometry(truth_state, &config->anchors[config->ref_anchor], &rdx, &rdy, &rdz, &rrho, &rrange);
    ...
    for (size_t i = 0; i < config->anchor_count; ++i) {
        ...
        if (config->enable_tdoa[i] && i != config->ref_anchor) {
            ...
            measurement->tdoa[i] = (range - rrange) / config->light_speed + gaussian_noise(config->tdoa_std_sec);
        }
