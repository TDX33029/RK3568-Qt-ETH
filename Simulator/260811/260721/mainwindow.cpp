    ...
    : QMainWindow(parent)
{
    ...
    // 实时 SPI 接收线程(Start Live 后才打开设备)
    m_spi = new SpiReader(this);
    m_liveTimer = new QTimer(this);
    m_liveTimer->setInterval(33);            // ~30 Hz UI 刷新
    connect(m_spi, &SpiReader::frameReceived, this, &MainWindow::onSpiFrame);
    connect(m_spi, &SpiReader::error, this, [this](const QString &msg){
        if (m_live) stopLive();
        QMessageBox::warning(this, QStringLiteral("SPI 错误"), msg);
    });
...
void MainWindow::buildMenuPage() {
    ...
    m_liveBtn = new QPushButton("Start Live (SPI)");
...
void MainWindow::startLive() {
    ...
    m_spi->setDevice(QStringLiteral("/dev/spidev1.0"));
    m_spi->setSpeedHz(8000000);
    m_spi->setPollPeriodMs(30);
    m_spi->start();
    m_liveTimer->start();
    updateResultDisplay();
    m_stack->setCurrentWidget(m_resultPage);
...
void MainWindow::stopLive() {
    ...
    m_liveBtn->setText(QStringLiteral("Start Live (SPI)"));
    if (m_hasResult) { tracker_free_result(&m_result); m_hasResult = false; }
}

void MainWindow::onSpiFrame(const Tracker3DMeasurement &meas, double dt_sec, quint16 seq) {
    if (!m_live) return;
...
void MainWindow::onLiveTick() {
    ...
    if (e) {
        fprintf(stderr, "[LIVE] seq=%u dt=%.2fms est=(%.3f,%.3f,%.3f) frames=%zu\n",
                m_liveSeq, m_liveDt * 1000.0, e[0], e[1], e[2], (size_t)m_result.steps);
        fflush(stderr);
    }
}
