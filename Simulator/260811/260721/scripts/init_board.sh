...
# Debian 10 (buster, EOL) 源修复: 主源已撤, 切到腾讯云(国内快) + 关闭过期校验
if ! grep -qE 'mirrors.cloud.tencent.com|archive.debian.org' /etc/apt/sources.list 2>/dev/null; then
    echo "   buster 源未换, 自动切到腾讯云..."
    $SUDO cp -a /etc/apt/sources.list "/etc/apt/sources.list.bak.$(date +%s)" 2>/dev/null || true
    $SUDO tee /etc/apt/sources.list >/dev/null <<'EOF'
deb http://mirrors.cloud.tencent.com/debian buster main
deb http://mirrors.cloud.tencent.com/debian-security buster/updates main
EOF
    $SUDO mkdir -p /etc/apt/apt.conf.d
    echo 'Acquire::Check-Valid-Until "false";' | $SUDO tee /etc/apt/apt.conf.d/99no-valid-until >/dev/null
    c_ok "apt 源已切到腾讯云 (回滚: /etc/apt/sources.list.bak.*)"
fi
